...
knitr::include_graphics('images/SGATmix.png', dpi = NA)
```
...
```{r, echo = FALSE}
load(paste0(wd, "/Results/", Species, "/", ID, "_SGATfit.RData"))
```
...
```{r fig.align='center', echo=FALSE, fig.width=7}
knitr::include_graphics('images/SGATmix2.png', dpi = NA)
```
...
```{r, echo = FALSE}
load(paste0(wd, "/Results/", Species, "/", ID, "_SGATGroupfit.RData"))
```
...
<span style="color:darkred">We again want to save the summary file as well as the MCMC Chains in case we want to summarize them differently in the future. We also need the chains to make maps with a density distribution or similar presentations of the results.</span>
